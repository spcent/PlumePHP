...
// 系统常量定义
defined('SINGLE_PHP_PATH') or define('SINGLE_PHP_PATH', __DIR__);
...
function U($route, array $params = array()) {
    ...
    if (empty($controller)) $controller = C('CONTROLLER');

    $module = array_pop($route);
    if (empty($module)) $module = C('MODULE');
...
class SinglePHP {
    ...
    public static function getInstance($conf) {    
        if (!(self::$_instance instanceof self)) {
            ...
            if (!isset($conf['APP_PATH'])) {
                $conf['APP_PATH'] = 'apps';
                if (!file_exists($cwd . DS . $conf['APP_PATH'])) {
                    halt('应用目录(APP_PATH)不存在!');
                }
            }

            C('APP_FULL_PATH', $cwd . DS . $conf['APP_PATH']);
            C('CONF_PATH', $cwd . (isset($conf['CONF_PATH']) ? $conf['CONF_PATH'] : DS . 'conf'));
            C('PUBLIC_PATH', $cwd . (isset($conf['PUBLIC_PATH']) ? $conf['PUBLIC_PATH'] : DS . 'public'));
            C('LIB_PATH', SINGLE_PHP_PATH . DS . 'Lib');

            // 加载全局使用函数
            includeIfExist(SINGLE_PHP_PATH . DS . 'common.php');
            if (file_exists(C('CONF_PATH') . DS . 'config.php')) {
                $config = require C('CONF_PATH') . DS . 'config.php';
                if (file_exists(C('CONF_PATH') . DS . 'config-'.ENVIRONMENT.'.php')) {
                    $envconf = require C('CONF_PATH') . DS . 'config-'.ENVIRONMENT.'.php';
    ...
    public function run() {
        ...
        // 设定错误和异常处理
        register_shutdown_function(array('SinglePHP', 'fatalError'));
        if (strcmp(strtoupper($pathMod), 'NORMAL') === 0 || !isset($_SERVER['PATH_INFO'])) {
            $this->m = isset($_GET['m']) ? ucfirst($_GET['m']) : 'Home';
            $this->c = isset($_GET['c']) ? ucfirst($_GET['c']) : 'Index';
            $this->a = isset($_GET['a']) ? ucfirst($_GET['a']) : 'Index';
        } else {
            $pathInfo = isset($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : '';
            $pathInfoArr = explode('/', trim($pathInfo, '/'));
            if (isset($pathInfoArr[0]) && $pathInfoArr[0] !== '') {
                $this->m = ucfirst($pathInfoArr[0]);
            } else {
                $this->m = 'Home';
            }
            
            if (isset($pathInfoArr[1]) && $pathInfoArr[1] !== '') {
                $this->c = ucfirst($pathInfoArr[1]);
            } else {
                $this->c = 'Index';
            }
            
            if (isset($pathInfoArr[2])) {
                $this->a = ucfirst($pathInfoArr[2]);
            } else {
                $this->a = 'Index';
            }
        }
        // 模块应用路径
        C('MODULE_PATH', C('APP_FULL_PATH') . DS . $this->m);
        // 加载全局使用函数
        includeIfExist(C('MODULE_PATH') . DS . 'functions.php');
    ...
    public static function autoload($class) {
        if (substr($class, -10) == 'Controller') {
            includeIfExist(C('MODULE_PATH') . DS .'Controller' . DS . $class . '.class.php');
        } else if (substr($class, -5) == 'Model') {
            includeIfExist(C('MODULE_PATH') . DS .'Model' . DS .$class . '.class.php');
        } elseif (substr($class, -6) == 'Widget') {
            includeIfExist(C('MODULE_PATH') . DS .'Widget' . DS .$class . '.class.php');
        } else {
            if (!file_exists(C('LIB_PATH') . DS . $class . '.class.php')) {
                // 文件名称可能全部小写
                $class = strtolower($class);
            }
            includeIfExist(C('LIB_PATH') . DS . $class . '.class.php');
        }
    }
    // 致命错误捕获
...
class View {
    ...
    public function __construct($tplDir = '') {
        if ($tplDir == '') {
            $this->_tplDir = C('MODULE_PATH') . DS .'View' .DS;
        }else{
            $this->_tplDir = $tplDir;
        }
    }
    ...
    public static function tplInclude($path, $data=array()) {
        self::$tmpData = array(
            'path' => C('MODULE_PATH') . DS .'View' .DS . $path . '.php',
            'data' => $data,
        );
        unset($path);
        unset($data);
...
class Widget {
    ...
    public function __construct() {
        ...
        $dir = C('MODULE_PATH') . DS .'Widget' . DS . 'Tpl' . DS;
...
class Log {
    ...
    public static function write($msg, $level = 'DEBUG', $wf = false) {
        if (function_exists('sae_debug')){ //如果是SAE，则使用sae_debug函数打日志
            ...
            if ($wf) {
                $logPath = C('MODULE_PATH') . DS . 'Log' . DS . date('Ymd') . '-error.log';
    ...
    public static function save() {
        ...
        $logPath = C('MODULE_PATH') . DS .'Log' . DS . date('Ymd') . '.log';
